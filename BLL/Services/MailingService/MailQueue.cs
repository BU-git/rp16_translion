...
namespace BLL.Services.MailingService
{
    ...
    internal sealed class MailQueue
    {
        ...
        /// </summary>
        private QueuedMessage FromMailMessage(MailMessage messageToCast)
            => new QueuedMessage(messageToCast, m_deletionDelta);

        #region private

        private static readonly Lazy<MailQueue> m_obj =
            new Lazy<MailQueue>(() => new MailQueue()); //singleton obj

        private readonly ConcurrentQueue<QueuedMessage> m_queue; //concurrent queue - main queue

        private readonly TimeSpan m_deletionDelta; //delta time of deletion
